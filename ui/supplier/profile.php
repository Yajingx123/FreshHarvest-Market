...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_profile') {
    ...
    if ($new_password !== '' || $confirm_password !== '') {
      ...
      if (empty($errors)) {
        // 安全地生成密码哈希
        $updateData['password_hash'] = md5($new_password);
      }
    }
...
<div class="profile-section">
    ...
    <div class="info-item" data-field="confirm_password">
...
function createEditInputs() {
    ...
    const passwordItem = document.querySelector('[data-field="password"]');
    if (passwordItem) {
        const input = document.createElement('input');
        input.type = 'password';
        input.name = 'new_password';
        input.placeholder = '请输入新密码（留空则不修改）';
    ...
    // 确认密码字段
    const confirmPasswordItem = document.querySelector('[data-field="confirm_password"]');
    if (confirmPasswordItem) {
        ...
        input.autocomplete = 'new-password';
        input.style.cssText = 'width: 100%; padding: 8px; margin-top: 5px; border: 1px solid #ddd; border-radius: 4px;';
        confirmPasswordItem.appendChild(input);
    }
...
// 在script标签的顶部添加这个函数（大约在第160行左右）
function showConfirmModal(title, message, options = {}) {
    return new Promise((resolve) => {
        // 创建弹窗HTML
        const modalHtml = `
            <div class="logout-modal-overlay" id="customConfirmModal" style="z-index:10000;">
                <div class="logout-modal">
                    <div class="logout-modal-icon" style="color:#3498db;">⚠️</div>
                    <h3 class="logout-modal-title">${title}</h3>
                    <p class="logout-modal-message">${message}</p>
                    <div class="logout-modal-actions">
                        <button type="button" class="logout-modal-btn logout-modal-cancel" id="confirmCancel">${options.cancelText || '取消'}</button>
                        <button type="button" class="logout-modal-btn logout-modal-confirm" id="confirmOk" style="background:linear-gradient(135deg, #3498db 0%, #2980b9 100%);">${options.okText || '确定'}</button>
                    </div>
                </div>
            </div>
        `;
...
function saveChanges() {
    ...
    .then(html => {
        ...
        if (password && confirmPassword && password.value.trim() && confirmPassword.value.trim()) {
            ...
            showConfirmModal(
                '密码修改成功',
                '为了账户安全，建议您重新登录。<br><br>点击"确认退出"立即退出，点击"取消"继续使用当前会话。',
                {
                    okText: '确认退出',
                    cancelText: '继续使用'
                }
